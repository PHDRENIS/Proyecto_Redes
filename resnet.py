...
EPSILON_END = 0.01
EPSILON_DECAY = 0.999
CHECKPOINT_PATH = 'pong_dqn_residual_checkpoint.pth'  # Cambio de nombre para distinguirlo
...
class DQNAgent:
    def __init__(self, load_checkpoint=False):
        ...
        if load_checkpoint and os.path.exists(CHECKPOINT_PATH):
            ...
            self.memory = checkpoint.get('memory', ReplayBuffer(MEMORY_CAPACITY))
...
try:
    ...
    for episode in range(agent.start_episode, 100):
        state = env.reset()
        episode_reward = 0
